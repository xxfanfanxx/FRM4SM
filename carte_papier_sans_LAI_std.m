...
for gh=[1:74]
  ...
  %  temp=zeros(length(datee),1);
    temp=std([datf;datfff;datms;datmm;datclaym;datsandr;datbde;datwp]);
...
colormap(t);
caxis([0 0.09]);
ylabel(tt,'std(ubRMSE) (m^3.m^{-3})');
m_coast('color',[0 0 0],'LineWidth',0.5);
hold on;
m_grid('linestyle','none','box','fancy','tickdir','in');
title({'Committed areas for the ubRMSE score (std range)','ubRMSE$=$f(FNO,FFO,FTS,FTM,SAND,CLAY,BulkD,FWP)','Trained on ISMN vs. SMOSL2 (t$\leq$30min, RFI$\leq$0.1, maxprobedepth$\leq$0.1m)'}, 'Interpreter', 'latex');
clear all;
